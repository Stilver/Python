...
os.chdir(r'd:\Coding\Python\Programs\LOCKS\Num')
...
class MainWindow:
    ...
    def _DoCreateIcons(self):
        ...
        iconPathName = os.path.join(os.getcwd(), 'NumOFF.ico') # set path to a tray icon
    ...
    def show_Num_state(self):
        while self.NLstate == win32api.GetKeyState(win32con.VK_NUMLOCK):
            ...
            if self.NLstate == -1:
                ...
                iconPathName = os.path.join(os.getcwd(), 'NumON.ico')
                self.NLstate = 1
            else:
                iconPathName = os.path.join(os.getcwd(), 'NumOFF.ico')
                self.NLstate = 0
