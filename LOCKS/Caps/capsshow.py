...
os.chdir(r'd:\Coding\Python\Programs\LOCKS\Caps')
...
class MainWindow:
    ...
    def _DoCreateIcons(self):
        ...
        iconPathName = os.path.join(os.getcwd(), 'CapsOFF.ico') # set path to a tray icon        
    ...
    def show_Caps_state(self):
        while self.CLstate == win32api.GetKeyState(win32con.VK_CAPITAL):
            ...
            if win32api.GetKeyState(win32con.VK_CAPITAL) == 1:
                iconPathName = os.path.join(os.getcwd(), 'CapsON.ico')
                self.CLstate = 1
            else:
                iconPathName = os.path.join(os.getcwd(), 'CapsOFF.ico')
                self.CLstate = 0
